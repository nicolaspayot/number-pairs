...
function App() {
  ...
  const [gameStatus, setGameStatus] = useState<"IDLE" | "IN_PROGRESS" | "WON">(
    "IDLE"
  );
      ...
      <div className="w-full bg-white/10 backdrop-blur-md rounded-2xl p-8 shadow-xl">
        <div className="flex justify-between items-center mb-4 p-6 rounded-lg bg-white/10 shadow-lg text-white">
          <div className="sm:text-2xl flex items-center gap-2">
            <Trophy />
            Moves:{" "}
            <span className="text-white font-bold min-w-[40px] inline-block">
              {totalMoves}
            </span>
          </div>
          <div className="sm:text-2xl flex items-center gap-2">
            <Clock />
            Time:{" "}
            <span className="text-white font-bold min-w-[86px] inline-block">
